defmodule MinimalTodo do
	...
	def create_headers do
		...
		create_header([])
		name = get_fields(%{})
		fields = Enum.map(title, &(field_from_user(&1)))
	...
	def add_todo(data) do
		...
		show_todos(new_data, false)
	end

	def get_command(data) do
		prompt = """
	Type the first letter of the command you want to run:
	"R)ead Todos    A)dd a Todo    D)elete Todo    L)oad a .csv    S)ave .csv
	"""
	...
	def get_item_name(data) do
		...
		if Map.has_key?(data, name) do
			IO.puts "Todo with this name already exists!\n"
			get_item_name(data)
		else
			name
		end
